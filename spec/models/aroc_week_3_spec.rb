...
# ----------------------
  ...
  it '16. returns the names of users who ordered one specific item' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    users = User.where(id: @item_8.orders.distinct.pluck(:user_id)).order(name: :asc).pluck(:name)
    # ------------------------------------------------------------
    ## User.where(id: @item_8.orders.distinct.pluck(:user_id)).pluck(:name).sort
  ...
  it '19. returns the average amount for all orders' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    # Solution goes here
    average = Order.average(:amount).round
    # ------------------------------------------------------------
